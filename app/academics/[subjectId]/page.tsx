...
async function getSubject(id: string): Promise<Subject | undefined> {
	try {
		...
		if (!res.ok) {
			...
			console.error(
				`API Error: Failed to fetch subject with status ${res.status}`
			);
			return undefined;
		}
				...
				const patched =
					fetchedSubject.courseCode === "SIID143_68"
						? {
								...
								title: fetchedSubject.title?.replace(
									/\bSIID?143[_\s/]?68\b/,
									"SIID143/68"
								),
						  }
			...
			<div className="flex h-screen items-center justify-center text-center">
				<div>
					...
					<p className="text-secondary mt-2">
						{error || "Could not load the subject."}
					</p>
					<Link
						href="/academics"
