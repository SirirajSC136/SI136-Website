    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md">
            ...
            <div className="space-y-4">
                {tasks.map(task => (
                    <div key={(task as any)._id} className="border p-4 rounded-lg flex justify-between items-start hover:bg-slate-50 transition-colors">
                        <div>
                        ...
                        <button onClick={() => handleDeleteTask((task as any)._id)} className="text-slate-400 hover:text-red-600 p-2 transition-colors">
                            <Trash2 size={18} />
                        </button>
                    </div>
                ))}
