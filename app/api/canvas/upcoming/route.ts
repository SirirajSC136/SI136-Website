...
const getTaskType = (title: string): 'assignment' | 'examination' => {
    ...
    return examKeywords.some(keyword => lowerCaseTitle.includes(keyword)) ? 'examination' : 'assignment';
};
...
export async function GET(request: Request) {
    ...
    try {
        const todoResponse = await fetch(`${canvasUrl}/users/self/todo`, { headers });
        ...
        const courseIds = [...new Set(rawTodos.map(item => item.assignment?.course_id).filter(id => id))];

        // 2. Fetch details for all those courses in parallel.
        const coursePromises = courseIds.map(id =>
            fetch(`${canvasUrl}/courses/${id}`, { headers }).then(res => res.ok ? res.json() : null)
        );
        const courseResults = await Promise.all(coursePromises);

        // 3. Create a quick-lookup map for course details.
        const courseDetailsMap = new Map<string, { name: string, course_code: string }>();
        courseResults.forEach(course => {
            if (course) {
                courseDetailsMap.set(course.id.toString(), { name: course.name, course_code: course.course_code });
            }
        });
        // --- END ENRICHMENT ---
