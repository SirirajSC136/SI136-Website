...
const ItemRenderer = ({ item }: { item: TopicItemData }) => {
	...
	switch (item.type) {
		case "Header":
			return (
				<h4 className="pt-5 pb-2 text-base font-bold text-primary">
					{item.title}
				</h4>
			);
...
const TopicItem = ({ topic }: { topic: Topic }) => {
	...
	// Filter items into groups: Headers, Special IDs (non-header), Regular IDs (non-header)
	const headerItems = topic.items.filter((item) => item.type === "Header");
	const contentItems = topic.items.filter((item) => item.type !== "Header");

	const specialItems = contentItems.filter((item) => /[^0-9]/.test(item.id));
	const regularItems = contentItems.filter((item) => !/[^0-9]/.test(item.id));

	// Combine them: Headers -> Special -> Regular
	const displayItems = [...headerItems, ...specialItems, ...regularItems];
	...
	return (
		<div className="overflow-hidden rounded-xl border border-border bg-background transition-shadow hover:shadow-md">
			<button
				...
				<ChevronDown
					className={`h-6 w-6 flex-shrink-0 text-secondary transition-transform duration-300 ${
						isOpen ? "rotate-180" : ""
					}`}
				/>
			</button>
